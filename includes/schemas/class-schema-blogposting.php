...
class KSSCTB_Schema_BlogPosting {
    public function generate($post_id, $settings) {
        ...
        if (!empty($settings['enable_comments'])) {
            ...
            if ($comment_count && isset($comment_count['approved']) && $comment_count['approved'] > 0) {
                $ordered_schema['commentCount'] = $comment_count['approved'];
            }
        }
